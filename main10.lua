...
btl = {
  ...
  },
  {
    ...
    orientation = "horizontal",
    {
      ...
      textSize = "18sp",
      text = getLS("L_Select_the_source_code"),
      ...
      for n = 1, #ls do
        ...
        if File(gg).isFile() then
          ...
          if err == nil then
            ...
            bh = getLS("L_Version") .. ": " .. tostring(wj:match('%appver="(.-)"'))
            bmh = getLS("L_The_package_name") .. ": " .. tostring(wj:match('%kagename="(.-)"'))
...
list.onItemClick = function(l, v, p, i)
  if File(v.Tag.gcv.Text).isDirectory() then
    if v.Tag.bt.Text ~= "nil" then
      activity.newActivity("main11", { v.Tag.gcv.Text, v.Tag.bt.Text, v.Tag.gcu.Text })
    end
  else
    SnackerBar.build()
